...
if(go7){
  go7.onclick=async function(){
    ...
    go7.disabled=true;
    try{
      out.innerHTML='<div class=\"c\">Dateien lesen…</div>';
      const parsed={};
      for(const key of Object.keys(slots)){
        out.innerHTML='<div class=\"c\">Lese '+escapeHtml(slots[key].name)+'</div>';
      ...
      out.innerHTML='<div class=\"c\">Sende kompaktes Paket…</div>';
      const controller=new AbortController();
      const timer=setTimeout(function(){controller.abort();},120000);
      const response=await fetch(\"/api/full7/predict\",{method:\"POST\",body:form,signal:controller.signal});
      clearTimeout(timer);
