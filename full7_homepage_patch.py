...
if(go7){
  go7.onclick=async function(){
    ...
    out.innerHTML='<div class="c">FULL-7 Contract prüft die sieben Quellen… bitte warten, Upload oft 20–90 Sekunden.</div>';
    try{
      ...
      out.innerHTML=
        ...
        '<p class="s">'+escapeHtml(ident.home_name||ident.home_id||"?")+' vs '+escapeHtml(ident.away_name||ident.away_id||"?")+
      ...
      out.innerHTML='<div class="c bad">Fehler: '+escapeHtml(String(error))+'. WLAN versuchen, Upload ist ca. 7 MB.</div>';
    }
  };
}
'''
