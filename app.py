...
def supplemental_report(match_data: Any, league_data: Any = None, form_data: Any = None, table_data: Any = None, player_data: Any = None, history_data: Any = None) -> Dict[str, Any]:
    ...
    return {
        ...
        "model_use": {
            ...
            "history": "Nur abgeschlossene Spiele derselben Competition vor dem Ziel-Anpfiff; bei vollständiger Paginierung wird die Dixon-Coles-Low-Score-Korrelation geschätzt.",
        },
    }
...
def _history_archive_audit(history_data: Any, filename: str, match_fields: Dict[str, Any]) -> Dict[str, Any]:
    """Validate HistoryDaten attribution and pagination before model fitting."""
...
def _analyze_bundle(parsed_files: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    history_fit = (result.get("diagnostics") or {}).get("history") or {}
    result["history_archive"]["model_use"] = bool(history_fit.get("active"))
    result["history_archive"]["model_reason"] = history_fit.get("reason")
...
<div class="w">
  <div class="c">
    ...
    <div class="s">Liga-relativer V5.5-Kern · HistoryDaten wirken über eine zeitgefilterte Dixon-Coles-Kalibrierung · keine Odds · INSUFFICIENT_DATA-Sperre und V5.2-Guardrails aktiv</div>
    ...
    <p class="s">Falls die Ordnerauswahl am iPhone nicht angeboten wird, wähle hier alle sechs JSON-Dateien gleichzeitig aus. Ohne HistoryDaten bleibt die abgesicherte v0.4.0-Basisberechnung aktiv.</p>
...
document.getElementById('go').onclick=async function(){
  ...
  try{
    ...
    const diag=data.diagnostics||{},goal=data.expected_goals||{},protocol=diag.elite_protocol||{},gates=protocol.gates||{},rvu=diag.result_vs_underlying_detail||{},history=diag.history||{},historyArchive=data.history_archive||{};
    ...
    out.innerHTML=
      ...
      '<div class="m"><div class="s">History-Modell</div><b>'+escapeHtml(history.active?'DIXON-COLES AKTIV':(history.received?'ERKANNT – NICHT AKTIV':'NICHT GELIEFERT'))+'</b><div class="s">ρ '+escapeHtml(history.rho==null?'—':history.rho)+' · '+escapeHtml(history.records_used==null?'0':history.records_used)+' Spiele</div></div>'+
      ...
      '<div class="m"><div class="s">Prüfstatus</div><div class="b">'+escapeHtml(historyArchive.status||'NICHT GELIEFERT')+'</div><div class="s">Erkannte Datensätze: '+escapeHtml(historyArchive.record_count_detected==null?'—':historyArchive.record_count_detected)+'</div></div>'+
      '<div class="m"><div class="s">Im Archiv</div><div class="b">'+escapeHtml(historyArchive.archived?'JA':'NEIN')+'</div></div>'+
      '<div class="m"><div class="s">Einfluss auf Modell</div><div class="b">'+escapeHtml(history.active?'JA':'NEIN')+'</div><div class="s">'+escapeHtml(history.reason||'Keine HistoryDaten geliefert.')+'</div></div>'+
      '</div></div>'+
